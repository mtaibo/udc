...
bool insertItem (tItemL d, tPosL p, tList *L) {
  ...
  if (lastPos == MAX - 1) return false; // Comprobación de si la lista es apta para añadir un elemento
  if (p == LNULL) p = lastPos + 1; // Si p == LNULL, la posición para añadir el elemento será al final
