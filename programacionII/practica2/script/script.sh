...
then
  ...
  ficherosEntrada=("")
  ficherosRef=("script_test/committee_list_ref" "script_test/project_list_ref")
fi
...
if [ "$MAIN_NAME" = "main" ]
then
  check_output "./main" 3 "committee_list.c project_list.c" ${VERBOSE}
  MAIN_OK=${allOK}
  CHECKPOINT_OK=${checkpoint}
  show_result "Checkpoint #2 result (April 24st):" ${CHECKPOINT_OK}
  show_result "Main global result:" ${MAIN_OK}
  
else
#10 de abril, primer seguimiento
  ficherosRef=("script_test/committee_list_ref")
  check_output "./test_committee_list" 0 "committee_list.c" ${VERBOSE}
  STACK_OK=${allOK}
  show_result "Committee list test:" ${STACK_OK}
  
  printf "\n"
  
  ficherosRef=("script_test/project_list_ref")
  check_output "./test_project_list" 0 "project_list.c" ${VERBOSE}
  LIST_OK=${allOK}
  show_result "Project list test:" ${LIST_OK}
  ...
  show_result "Committee list test:" ${STACK_OK}
  show_result "Project list test:" ${LIST_OK}
  show_result "Tests global result (checkpoint #1 - April 10th):" $((${STACK_OK} * ${LIST_OK}))
